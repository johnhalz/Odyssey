...
from .models import Equipment, Hardware, HardwareModel, Order
from .serializers import (
    EquipmentSerializer,
    HardwareModelSerializer,
    HardwareSerializer,
    OrderSerializer,
)
...
class HardwareList(ModelListAPIView):
    ...
    @extend_schema(
        ...
        responses={
            200: PaginatedSerializer.create_paginated_serializer(HardwareSerializer)
        },
    )
...
class OrderList(ModelListAPIView):
    ...
    @extend_schema(
        parameters=open_api_params,
        responses={
            200: PaginatedSerializer.create_paginated_serializer(OrderSerializer)
        },
    )
    def get(self, request):
        return super().get(request)
...
class EquipmentList(ModelListAPIView):
    ...
    open_api_params = [
        ...
        OpenApiParameter(
            name="calibrated_after", required=False, type=OpenApiTypes.DATETIME
        ),
        OpenApiParameter(
            name="calibrated_before", required=False, type=OpenApiTypes.DATETIME
        ),
        OpenApiParameter(
            name="created_after", required=False, type=OpenApiTypes.DATETIME
        ),
    ...
    @extend_schema(
        parameters=open_api_params,
        responses={
            200: PaginatedSerializer.create_paginated_serializer(EquipmentSerializer)
        },
    )
    def get(self, request):
        return super().get(request)
    ...
    def _filter(self, request) -> list[Equipment] | Response:
        ...
        for param, value in request.GET.items():
            if param == "id":
                ...
                calibrated_before_dt = datetime.strptime(
